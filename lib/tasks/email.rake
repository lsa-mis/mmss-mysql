...
namespace :email do
  # Usage: bundle exec rake email:test[your@email.com]
  # Note: If using zsh, wrap the email in quotes: bundle exec rake 'email:test[your@email.com]'
  desc 'Send a test email to the specified address'
  task :test, [:email] => :environment do |_t, args|
    if args[:email].blank?
      puts 'Please provide an email address: bundle exec rake email:test[your@email.com]'
      exit 1
    end
